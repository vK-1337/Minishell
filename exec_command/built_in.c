...
/*   Updated: 2024/04/03 17:27:50 by vda-conc         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_in_redirections_built_in(t_ast *command, t_exec **exec,
		t_list **env_list)
{
	t_token	*travel;
	...
	(*exec)->fd[0] = c_fd_in((*exec)->fd[0], travel->token, travel->file_redir);
	if ((*exec)->fd[0] == -1)
		return (-1);
...
int	manage_built_in2(char **brut_input, t_list **env_list, t_ast *ast)
{
	...
	if (exit_status == 2)
		return (free(*brut_input), ft_exit(command, env_list));
